...
static int8_t stm32f10x_gpio_get(gpio_desp_t *gpio_desp)
{
    ...
    *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) &= ~(0xff<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
    *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= ((0x03)<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
...
static void  stm32f10x_gpio_set_mode(gpio_desp_t *gpio_desp, gpio_mode_t mode)
{
    ...
    *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) &= ~(0xff<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
    switch(mode)
    {
        ...
        break;
        case GPIO_OUT_OD:
            *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= ((0x07)<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
        break;
        case GPIO_OUT_FLEX_PP:
            *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= ((0xB)<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
        break;
        case GPIO_OUT_FLEX_OD:
            *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= ((0xF)<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
        break; 
        case GPIO_OUT_PP_UP:
            *(uint64_t*)(GPIOA_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= ((0x03)<<(GPIO_NUM2PORT(gpio_desp->pin_num)*4));
...
{
    *(uint32_t*)(GPIOA_BASE + 0x0C + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) &= ~(1<<GPIO_NUM2PIN(gpio_desp->pin_num));
    *(uint32_t*)(GPIOA_BASE + 0x0C + (GPIO_NUM2PORT(gpio_desp->pin_num))*0x400) |= (value<<GPIO_NUM2PIN(gpio_desp->pin_num));
}
