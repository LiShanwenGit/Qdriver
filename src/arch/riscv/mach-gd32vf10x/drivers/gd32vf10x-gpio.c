...
static int8_t gd32vf_gpio_get(gpio_desp_t *gpio_desp)
{
    ...
    //clear CTL
    *((volatile uint64_t*)(GPIO_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num)*0x400U))) &= 
    ~(0xF<<(GPIO_NUM2PIN(gpio_desp->pin_num)*4));
    //set CTL  output (default)
    *((volatile uint64_t*)(GPIO_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num)*0x400U))) |= 
    (0x3<<(GPIO_NUM2PIN(gpio_desp->pin_num)*4));
    //set 0CTL output (default)
    *((volatile uint32_t*)(GPIO_BASE + 0x0C + (GPIO_NUM2PORT(gpio_desp->pin_num)*0x400U))) &= 
    ~(1<<GPIO_NUM2PIN(gpio_desp->pin_num));
}

static void  gd32vf_gpio_set_mode(gpio_desp_t *gpio_desp, gpio_mode_t mode)
{
    //clear CTL
    *((volatile uint64_t*)(GPIO_BASE + (GPIO_NUM2PORT(gpio_desp->pin_num)*0x400U))) &= 
    ~(0xF<<(GPIO_NUM2PIN(gpio_desp->pin_num)*4));
...
{
    return *((volatile uint32_t*)(GPIO_BASE + 0x0C + (GPIO_NUM2PORT(gpio_desp->pin_num)*0x400U)));
}
